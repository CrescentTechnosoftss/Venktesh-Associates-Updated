...
/*!*********************************************!*\
  !*** ./resources/js/masters/user-master.js ***!
  \*********************************************/
(function () {
  ...
  function getUserDetails() {
    ...
    if (user !== '') {
      axios.get(`${currentUrl}/${user}`).then(response => {
  ...
  function updateUser() {
    ...
    if (user === '') {
      ...
      axios.patch(`${currentUrl}/${user}`, data).then(response => {
        if (response.data.status) {
          toastr.success(response.data.message);
        } else {
          toastr.error(response.data.message);
  ...
  function deleteUser() {
    ...
    if (user === '') {
      ...
      deleteConfirmation('User').then(swalResult => {
  ...
  function clear() {
    ...
    axios.get(`${currentUrl}/users`).then(response => {
      const options = [];
