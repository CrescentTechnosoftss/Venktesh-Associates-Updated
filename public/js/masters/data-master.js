...
/*!*********************************************!*\
  !*** ./resources/js/masters/data-master.js ***!
  \*********************************************/
(function () {
  "use strict";
  ...
  $('#btnRemoveDeliveryBoy').on('click', removeDeliveryBoy); //Autocompletes
  ...
  $('#txtEngineerName').autocomplete({
    source: engineers
  });
  $('#txtDeliveryBoy').autocomplete({
    source: deliveryBoys
  });
  ...
  function removeBankName() {
    ...
    if (bankNames.includes(bankName)) {
      axios.delete(`${currentUrl}/bank/${encodeURIComponent(bankName)}`).then(d => {
  ...
  function removeBranchName() {
    ...
    if (branchNames.includes(branchName)) {
      axios.delete(`${currentUrl}/branch/${encodeURIComponent(branchName)}`).then(d => {
  ...
  function addDeliveryBoy() {
    ...
    if (deliveryBoy !== '' && deliveryBoys.includes(deliveryBoy) === false) {
      axios.post(`${currentUrl}/delivery-boy`, {
        deliveryBoy
      }).then(d => {
  ...
  function removeDeliveryBoy() {
    ...
    if (deliveryBoys.includes(deliveryBoy)) {
      axios.delete(`${currentUrl}/delivery-boy/${encodeURIComponent(deliveryBoy)}`).then(d => {
