(function () {
    ...
    $('#btnDownload').on('click', function() {
        ...
        const gstTypes = $('#GstType').val();
        downloadInvoice(fromDate, toDate,gstTypes);
      });
    ...
    function downloadInvoice(fromDate, toDate, gstTypes) {
        if (fromDate === '' || toDate === '') {
            toastr.info('Select the From and To Dates to Download');
        } else {
                        const actualUrl = downloadUrl
                .replace(':from', fromDate)
                .replace(':to', toDate)
                .replace(':gstType', gstTypes);
