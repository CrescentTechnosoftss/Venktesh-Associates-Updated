...
"use strict";
/*!******************************************************!*\
  !*** ./resources/js/reports/sales-report-by-file.js ***!
  \******************************************************/
...
(function () {
  ...
  const dataTable = $('#tblSalesReportByFile').DataTable({
    ...
    footerCallback: (tfoot, data) => {
      ...
      const approved = data.filter(f => f.paymentStatus.text === 'Paid & Approved').reduce((prev, curr) => prev + curr.paidAmount, 0);
    ...
    columns: [{
      ...
      render: data => `<label style="color:${data.color}">${data.text}</label>`
    }, {
      data: 'engineerName',
      title: 'Engineer'
    }, {
