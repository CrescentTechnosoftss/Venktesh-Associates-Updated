...
$pdf->cell(190, 5, 'Invoice Bill', 0, 1, 'C');
$pdf->setX(20);
...
$pdf->cell(120, 6, 'To');
$pdf->cell(23, 6, 'Invoice No');
$pdf->cell(30, 6, ': ' . $data->payment->bill_no, 0, 1);


$pdf->cell(120, 6, 'THE MANAGER,');
$pdf->cell(23, 6, 'Invoice Date');
$pdf->cell(30, 6, ': ' . date('d-m-Y', strtotime($data->payment->InvoiceDate)), 0, 1);
...
if ($data->IGST) {
    ...
    $pdf->cell(20, 6, 'Rs.' . number_format((float)$Igst_amount) . '/-', 0, 1, 'R');
    ...
    $pdf->cell(30, 6, number_format($data->payment->amount_paid + $Igst_amount) . '/-', 0, 1, 'R');
    ...
    $pdf->cell(135, 6, 'Grand Total', 0, 0, 'R');
    // $pdf->cell(30, 6, number_format($data->total_amount) . '/-', 0, 1, 'R');
    $pdf->cell(30, 6, number_format($data->payment->amount_paid + $half_gst_amount + $half_gst_amounts) . '/-', 0, 1, 'R');
...
// $pdf->cell(135, 6, 'Grand Total', 0, 0, 'R');
// // $pdf->cell(30, 6, number_format($data->total_amount) . '/-', 0, 1, 'R');
// $pdf->cell(30, 6, number_format($data->payment->amount_paid + $half_gst_amount + $half_gst_amount) . '/-', 0, 1, 'R');


// $pdf->ln(3);
// // $inWords = strtoupper(getIndianCurrency($data->total_amount)) . ' ONLY';
// $inWords = strtoupper(getIndianCurrency($data->payment->amount_paid + $half_gst_amount + $half_gst_amount)) . ' ONLY';
// $pdf->cell(190, 5, 'Rupees in words ' . $inWords, 0, 1);
