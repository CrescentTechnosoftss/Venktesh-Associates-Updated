...
<form method="POST" action="{{route('exportPaidInvoices')}}" id="frmExport" target="_blank">
    @csrf
...
<script>
    const currentUrl = "{{route('paidInvoiceReports')}}";
    // const downloadUrl = "{{url('download-invoices')}}";
    const downloadUrl = "{{ route('downloadMultiple', ['from' => ':from', 'to' => ':to']) }}";
</script>
<script src="{{asset('js/reports/paid-invoices.js')}}"></script>
@endsection