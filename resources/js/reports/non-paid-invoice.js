...
const dataTable = $('#tblNonPaidInvoice').DataTable({
    ...
    columns: [
        ...
        { title: 'Proforma Date', data: 'ProformaDate', render: formatDateForBrowser },
        { title: 'Proforma No', data: 'ProformaNo' },
        { title: 'File No', data: 'fileNo' },
        { title: 'Bank / Branch', data: 'bank' },
        { title: 'Customer Name / Phone No', data: 'customer' },
        { title: 'Purchaser', data: 'purchaser' },
        { title: 'Bill Amount', data: 'billAmount' },
        { title: 'Referred By', data: 'referredBy' },
        { title: 'Engineer', data: 'engineer' },
        { title: 'Send Mail', render: () => `<button class="btn btn-primary btn-sm rounded"><i class="fa fa-envelope"></i></button>` },
        {
            title: 'Customer & Bank',
...
function checkMailStatus() {
    axios.get(`${currentUrl}/bank-emails`)
        .then(response => {
...
function sendBulkMail() {
    ...
    if (data.bank === 'all') {
        ...
        axios.post(`${currentUrl}/send-bulk-mail`, data)
            .then(response => {
                if (response.data.status) {
                    toastr.success(response.data.message);
                }
            ...
            }).then(() => { $('#btnMail').prop('disabled', false).text('Send Mail'); });
    }
}
